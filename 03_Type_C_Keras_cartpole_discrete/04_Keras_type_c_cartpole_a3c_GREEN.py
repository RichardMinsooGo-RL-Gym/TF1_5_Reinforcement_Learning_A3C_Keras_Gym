...
class RL_Agent:
    ...
    def loss_and_train(self):
        ...
        log_lik = K.log(log_p + 1e-10) * K.stop_gradient(td_error)
        # log_lik = -K.sum(log_lik)
        actor_loss = -K.mean(K.sum(log_lik))
                
        # entropy(for more exploration)
        # entropy = K.sum(policy * K.log(policy + 1e-10), axis=1)
        entropy = -K.mean(K.sum(policy * K.log(policy + 1e-10), axis=1))
...
if __name__ == "__main__":
    ...
    if os.path.isfile("./save_model/Cartpole_model.h5"):
        global_agent.model.load_weights("./save_model/Cartpole_model.h5")
        print(" Model restored!!")
        
    global_agent.train()
    
    # Save the trained results
    global_agent.model.save_weights("./save_model/Cartpole_model.h5")
