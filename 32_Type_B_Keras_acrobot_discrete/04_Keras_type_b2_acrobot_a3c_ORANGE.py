...
class A3CAgent:
    ...
    def _init_op(self, model):
        ...
        log_prob = tf.reduce_sum(policy * action, axis=1, keep_dims=True)

        td_error = q_target - value

        # maximize policy
        actor_loss = - tf.stop_gradient(td_error) * tf.log(log_prob + 1e-10)
        
        # train_op value error
        critic_loss = self.c_loss_weight * tf.square(td_error)
        
        # maximize entropy (regularization)
        entropy = self.entropy_panelty_weight * tf.reduce_sum(policy * tf.log(policy + 1e-10), axis=1, keep_dims=True)
        
        loss_total = tf.reduce_mean(actor_loss + critic_loss + entropy)
...
class Agent(threading.Thread):
    ...
    def work(self):
        ...
        while True and scores < 2000:
            ...
            if done or scores == 2000: # terminal state
                next_state = None

            self.train_agent(state, action, reward, next_state)

            state = next_state
            scores += 1

            if done or scores == 2000:
                episode += 1
                break
