...
class RL_Agent:
    def __init__(self, state_size, action_size, env_name):
        ...
        self.loss_and_train = [self.actor_optimizer(), self.critic_optimizer()]
    ...
    def build_critic(self):
        critic = Sequential()
        critic.add(Dense(self.hidden1, input_dim=self.state_size, activation='relu', kernel_initializer='glorot_uniform'))
        critic.add(Dense(self.hidden2, input_dim=self.state_size, activation='relu', kernel_initializer='glorot_uniform'))
    ...
    def train(self):
        ...
        agents = [Worker(i, self.actor, self.critic, self.loss_and_train, self.env_name, 
...
class Worker(threading.Thread):
    def __init__(self, index, actor, critic, loss_and_train, env_name, action_size, state_size):
    ...
    def train_model(self):
        ...
        values = self.critic.predict(np.array(self.buffer_state))
        values = np.reshape(values, len(values))

        discounted_rewards = discounted_rewards - values

        self.loss_and_train[0]([self.buffer_state, self.buffer_action, discounted_rewards])
        self.loss_and_train[1]([self.buffer_state, discounted_rewards])
        self.buffer_state, self.buffer_action, self.buffer_reward = [], [], []
...
if __name__ == "__main__":
    global_agent = RL_Agent(state_size, action_size, env_name)
